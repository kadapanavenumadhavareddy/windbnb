...
"
  ...
  <FilterDrawer
    v-if="Displaydrawer"
    :tab="active"
    @filterdraweroff="Filter('')"
  />
  <footer>created by Venu Madhava Reddy Kadapana - devChallenges.io</footer>
</template>
<script>
...
export default {
  ...
  computed: {
    ...mapState({ stay: "selectedstays", sp: "selectedplace" }),
  },
  methods: {
    Filter(val) {
      this.active = val;
  ...
  created() {
    this.$store.dispatch("places");
    if (this.$store.state.selectedplace == null) {
      this.selectedplaces = this.$store.state.places[0];
    } else {
      this.selectedplaces = this.$store.state.selectedplace;
    }
    this.$store.dispatch("selectedhotels", this.selectedplaces);
  },
};
</script>
<style>
...
@media screen and (min-width: 900px) {
  .display-properties {
    padding: 32px 62px 32px 94px;
  }
  .card {
    padding: 0px 32px 33px 0px;
  }
  .stay {
    padding: 67px 94px 0px 94px;
  }
  .nav {
    margin: 32px 94px 0px 94px;
  }
}
