...
let store = createStore({
  ...
  actions: {
    places(state) {
      state.state.stays.forEach((element) => {
        let some = state.state.places.find((val) => {
          return val.city == element.city;
        });
        if (some == undefined) {
          state.commit("pla", { city: element.city, country: element.country });
        } else {
          let some1 = state.state.places.find((val) => {
            return val.country == element.country;
          });
          if (some1 == undefined) {
            state.commit("pla", {
              city: element.city,
    ...
    selectedhotels(context, playload) {
      context.state.selectedplace = playload;
      var selectedlocation = playload;
      let some = context.state.stays.filter((val) => {
        if (
          val.city == selectedlocation.city &&
          val.country == selectedlocation.country
        ) {
          return val;
        }
      });
      context.commit("selectedstay", some);
    },
    search(context, playload) {
      let some;
      if (playload.adults + playload.childrens > 0) {
        some = context.state.stays.filter((val) => {
          if (
            val.city == playload.city &&
            val.country == playload.country &&
            val.maxGuests >= playload.adults + playload.childrens
          ) {
            return val;
          }
        });
        ...
        some = context.state.stays.filter((val) => {
          if (val.city == playload.city && val.country == playload.country) {
            return val;
          }
        });
      }
  ...
  mutations: {
    pla(state, pau) {
      state.places.push(pau);
    },
    selceted(state, place) {
      state.selectedplace = place;
    },
    selectedstay(state, stays) {
      state.selectedstays = stays;
    },
  },
});
app.use(store);
